<?php
/*
 * Common configuration that can be used throughout the application Please refer to Config class in the API doc for a complete list of configurations Access via Singleton, eg. Q::conf()->BASE_PATH;
 */
error_reporting ( E_ALL | E_STRICT );
date_default_timezone_set ( 'Asia/Shanghai' );

/**
 * for benchmark purpose, call Q::benchmark() for time used.
 */
// $config['START_TIME'] = microtime(true);

// For framework use. Must be defined. Use full absolute paths and end them with '/' eg. /var/www/project/
$config ['SITE_PATH'] = realpath ( '..' ) . DIRECTORY_SEPARATOR . 'home' . DIRECTORY_SEPARATOR;
// $config['PROTECTED_FOLDER'] = 'protected/';
$config ['BASE_PATH'] = realpath ( '..' ) . DIRECTORY_SEPARATOR . 'framework' . DIRECTORY_SEPARATOR;
...
$config ['SUBFOLDER'] = str_replace ( $_SERVER ['DOCUMENT_ROOT'], '', str_replace ( '\\', DIRECTORY_SEPARATOR, $config ['SITE_PATH'] ) );
 ...
 * This is needed for writing log files and using the log viewer tool
 */
